...
function Step({ n, t, d }: { n: string; t: string; d: string }) {
  return (<div className="step"><div className="step-n">{n}</div><div className="step-t">{t}</div><div className="step-d">{d}</div></div>);
}
...
// The firewall dashboard: live pool state, submit a request, recent requests.
export default function Firewall() {
  ...
  const { data: balances } = useReadContracts({
    contracts: (holderAddrs as string[] | undefined)?.map((a) => ({ address: ASSET_ADDRESS as `0x${string}`, abi: ASSET_ABI, functionName: "balanceOf", args: [a as `0x${string}`] })) ?? [],
  });
      ...
      try {
        ...
        for (let id = n; id >= start; id--) {
          try {
            ...
            out.push({ id, status: Number(r.status), amount: r.amount, type: Number(rating(r)) });
          } catch { /* skip */ }
        }
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      setTimeout(() => {
        writeContract({ address: GATE_ADDRESS as `0x${string}`, abi: GATE_ABI, functionName: kind === "TRANSFER" ? "requestTransfer" : "requestRedeem", args: kind === "TRANSFER" ? [recipient as `0x${string}`, amt] : [amt] });
      ...
      <main className="grid">
        ...
        <section className="card action">
          ...
          <form onSubmit={submit}>
            ...
            <label>Amount ({SLUICE_META.assetSymbol})
...
// helper to read requestType from getRequest tuple (kept local to avoid tuple typing churn)
function rating(r: any): number {
  return r.requestType ?? 0;
}
