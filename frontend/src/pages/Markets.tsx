...
import {
  ...
  listDreamMarkets, marketCategory, marketLabel, minutesLeft, probabilityFromBook, safeSizeConstraint, depthLadder,
} from "../dreamdex";
import { scaledNumber } from "../units";
import RiskDimensionsDisplay from "../components/RiskDimensions";
import DepthLadderDisplay from "../components/DepthLadderDisplay";

const DEFAULT_DIMENSIONS = {
  market: { name: "Market", score: 0, status: "pass" as const },
  liquidity: { name: "Liquidity", score: 0, status: "pass" as const },
  exposure: { name: "Exposure", score: 0, status: "pass" as const },
  collateral: { name: "Collateral", score: 0, status: "pass" as const },
  control: { name: "Control", score: 0, status: "pass" as const },
};
...
export default function Markets() {
  ...
  const safeSizeBinding = useMemo(() => {
    if (!selected || !walletSnapshot || safeSize <= 0) return null;
    return safeSizeConstraint(selected, book, safeSize, price, side, walletSnapshot, { maxCost: side === "buy" ? lossBudget : undefined });
  ...
  const ladder = useMemo(() => depthLadder(book, side), [book, side]);
  ...
  async function execute() {
    ...
    try {
      ...
      if (hash) record("info", "View on-chain", `Transaction ${hash.slice(0, 10)}... on Somnia explorer; portfolio auto-syncs`);
