...
export default function ScenarioSimulator({ pool }: { pool: { holders: { address: string; balance: bigint; pct: number }[]; totalSupply: bigint } }) {
  const target = [...pool.holders]
    .filter((holder) => holder.balance > 0n && (!DEMO_ATTACKER_ADDRESS || holder.address.toLowerCase() !== DEMO_ATTACKER_ADDRESS.toLowerCase()) && holder.address.toLowerCase() !== GATE_ADDRESS.toLowerCase())
  ...
  const preview: Projection | null = DEMO_ATTACKER_ADDRESS && target && pool.totalSupply > 0n
      ...
      <div className="sim-grid">
        <div className="sim-controls">
          <div className="sim-params">
            ...
            <div><span>From</span><b>{DEMO_ATTACKER_ADDRESS ? `${shortAddr(DEMO_ATTACKER_ADDRESS)} (synthetic demo attacker)` : "Operator demo address not configured"}</b></div>
            <div><span>To</span><b>{target ? `${shortAddr(target)} (live largest holder)` : "No target available"}</b></div>
          </div>
          <button className="danger big" disabled>
            {DEMO_ATTACKER_ADDRESS ? "Breach test operator-only" : "Breach test unavailable"}
          </button>
