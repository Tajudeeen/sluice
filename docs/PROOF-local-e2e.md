...
```
Pool seeded: 1,900,000 SLUSD total (1,000,000 base across 6 holders, largest A =
35%; +900,000 synthetic SLUSD to the demo attacker).
...
[sluice-agent] req 2 BLOCK | score=32 hardBlock=true reason=LARGEST_HOLDER_LIMIT: projected top holder 65.8% >= 50%
...
request #2: type=TRANSFER amount=900000.0 status=BLOCKED  (concentration ATTACK)
```
...
- Risk engine: deterministic concentration (HHI / largest-holder) hard-block fired on a real 65.8% projection.
...
The legacy escrow proof is local-only. The hackathon product uses the live
DreamDEX indexer and Somnia Shannon testnet for market discovery and trading.
